...
function DayMonthYearPicker({
  ...
  const blanks = Array.from(
    { length: getFirstDay(calYear, calMonth) },
    (_, i) => i,
  );
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      {open && (
        <div
          className="absolute z-50 mt-1 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg shadow-lg p-4"
          style={{ minWidth: 280 }}
        >
            ...
            <>
              <div className="grid grid-cols-7 mb-1">
                {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
                  <div
                    key={d}
                    className="text-center text-xs font-medium py-1 text-[#A9B4CD] dark:text-slate-500"
                  >
                    {d}
                  </div>
                ))}
...
export default function Step1GeneralInfo({
  ...
  const handleChange = (field: keyof FormFields, value: string | string[]) => {
    ...
    if (field === "internshipStart" || field === "internshipEnd") {
      const start =
        field === "internshipStart"
          ? (value as string)
          : updated.internshipStart;
      const end =
        field === "internshipEnd" ? (value as string) : updated.internshipEnd;
      updated.internshipPeriod =
        ...
        <div className="flex-1 space-y-3 md:space-y-5">
            ...
            <div className="flex gap-4">
              {["Male", "Female"].map((g) => (
                <label key={g} className="flex items-center cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-[#1C2D4F] dark:text-slate-300">
                    {g}
                  </span>
                </label>
              ))}
          ...
          <div className="min-w-0">
            ...
            <div className="flex min-w-0 gap-2">
              ...
              {/* Input เบอร์โทรศัพท์ (กรอกได้เฉพาะตัวเลขและขีด) */}
              <input
                type="tel"
                maxLength={15}
                value={fields.phoneNumber.replace(/^\+\d+\s?/, "")} // แสดงเฉพาะเบอร์ ไม่รวมรหัสประเทศในช่องกรอก
                onChange={(e) => {
                  const sanitizedValue = e.target.value.replace(/[^0-9-]/g, "");
                  // บันทึกรหัสประเทศ + เบอร์โทร ลงในฐานข้อมูล
                  handleChange(
                    "phoneNumber",
                    `${phonePrefix} ${sanitizedValue}`,
                  );
                }}
