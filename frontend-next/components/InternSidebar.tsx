...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const missingSkillsCount = useMemo(() => {
    ...
    const userSkillNames = new Set(
      (profileData.skills || []).map((s: any) => s.name.toLowerCase().trim()),
    );
    const evidenceSkills = new Set<string>();
    (profileData.certificates || []).forEach((c: any) => {
      (c.relatedSkills || c.tags || []).forEach((s: string) =>
        evidenceSkills.add(s.toLowerCase().trim()),
      );
    });
    (profileData.projects || []).forEach((p: any) => {
      (p.relatedSkills || p.skills || []).forEach((s: string) =>
        evidenceSkills.add(s.toLowerCase().trim()),
      );
    });
    let count = 0;
      ...
      `}>
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto custom-scrollbar">
            ...
            <div className="pt-6 lg:hidden border-t border-slate-100 dark:border-slate-800 mt-4">
              <p className="px-4 text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Menu</p>
              
              <Link href="/intern/find-companies" className="group px-4 py-3 rounded-xl flex items-center space-x-3 transition-all mt-1 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
                ...
                <span className="font-bold">Find Companies</span>
              </Link>

              <Link href="/intern/messages" className="group px-4 py-3 rounded-xl flex items-center space-x-3 transition-all mt-1 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
              ...
              <button
                onClick={() => { onClose?.(); setIsBugModalOpen(true); }}
                className="w-full group px-4 py-3 rounded-xl flex items-center space-x-3 transition-all mt-1 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
