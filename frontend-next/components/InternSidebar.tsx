...
// เพิ่ม props interface สำหรับ Responsive
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  // ปิด Sidebar อัตโนมัติเมื่อมีการเปลี่ยนหน้า (สำหรับมือถือ)
  useEffect(() => {
    if (onClose) onClose();
  }, [pathname]);
  ...
  return (
    <>
      ...
      <aside className={`
        ...
        w-64 bg-white dark:bg-slate-900 min-h-screen pt-8 border-r border-slate-200 dark:border-slate-800 
        shrink-0 transition-transform duration-300 ease-in-out
        ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* ปุ่มกากบาทปิด Sidebar (เฉพาะมือถือ) */}
        <button 
          onClick={onClose}
          className="lg:hidden absolute top-4 right-4 p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"
        >
        ...
        <div className="px-4 space-y-1.5 h-full overflow-y-auto">
          {/* Profile Group */}
          <div className="space-y-1">
            <div className={`group w-full px-4 py-3 rounded-xl flex items-center justify-between transition-all duration-200 ${
              ...
              >
                <svg className={`w-4 h-4 transition-transform duration-300 ${isProfileDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}>
            ...
            {isProfileDropdownOpen && (
              <div className="ml-4 pl-2 border-l-2 border-slate-100 dark:border-slate-800 space-y-1 mt-1">
                <Link href="/intern/project" className={`flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${isProjectPage ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                ...
                <Link href="/intern/skills" className={`flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${isSkillPage ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                  <div className="flex items-center space-x-3">
                  ...
                  {missingSkillsCount > 0 && (
                    <span className="bg-rose-500 text-white text-[10px] px-2 py-0.5 rounded-full font-black animate-pulse">{missingSkillsCount}</span>
                  )}
                </Link>

                <Link href="/intern/certificates" className={`flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${isCertificatesPage ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                  <div className="flex items-center space-x-3">
          ...
          <div className="pt-4 pb-2">
            <p className="px-4 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 dark:text-slate-600 mb-2">Discovery</p>
            <Link href="/intern/ai-job-match" className={`group px-4 py-3 rounded-xl flex items-center space-x-3 transition-all ${isAiMatchPage ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
              <svg className={`w-5 h-5 ${isAiMatchPage ? 'text-white' : 'text-slate-400 dark:text-slate-500 group-hover:text-blue-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              <span className="font-bold tracking-tight">AI Job Match</span>
            </Link>
            <Link href="/intern/applied" className={`group px-4 py-3 rounded-xl flex items-center space-x-3 transition-all mt-1 ${isAppliedPage ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
              <svg className={`w-5 h-5 ${isAppliedPage ? 'text-white' : 'text-slate-400 dark:text-slate-500 group-hover:text-blue-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              <span className="font-bold tracking-tight">Applied</span>
            </Link>
            <Link href="/intern/bookmark" className={`group px-4 py-3 rounded-xl flex items-center space-x-3 transition-all mt-1 ${isBookmarkPage ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
              <svg className={`w-5 h-5 ${isBookmarkPage ? 'text-white' : 'text-slate-400 dark:text-slate-500 group-hover:text-blue-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" /></svg>
