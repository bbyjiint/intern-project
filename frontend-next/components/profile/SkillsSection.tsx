  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm p-6 mb-6 border border-slate-100 dark:border-slate-800 transition-colors">
      ...
      {missingSkills.length > 0 && (
        <div className="mb-8 p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-amber-600/80 dark:text-amber-400/80 mt-1">
                You have {missingSkills.length} skills (e.g. {missingSkills.slice(0, 3).join(', ')}{missingSkills.length > 3 ? '...' : ''}) in your evidence that aren't on your profile yet.
              </p>
            </div>
          </div>
          <button
            onClick={() => router.push("/intern/skills")}
            className="w-full sm:w-auto px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-md shadow-amber-500/20 active:scale-95 whitespace-nowrap"
          >
            Review & Add
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-2xl p-6">
          <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-8 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-500" /> Technical Skills</h3>
          {technicalSkills.length > 0 ? (technicalSkills.map((s) => <SkillItem key={s.id} skill={s} />)) : (<p className="text-slate-400 dark:text-slate-600 italic text-sm text-center py-4 font-medium underline underline-offset-4 decoration-dotted">No technical skills added</p>)}
        </div>

        <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-2xl p-6">
          <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-8 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500" /> Business / Soft Skills</h3>
          {businessSkills.length > 0 ? (businessSkills.map((s) => <SkillItem key={s.id} skill={s} />)) : (<p className="text-slate-400 dark:text-slate-600 italic text-sm text-center py-4 font-medium underline underline-offset-4 decoration-dotted">No business skills added</p>)}
        </div>

        {otherSkills.length > 0 && (
          <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-2xl p-6">
            <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-8 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-violet-500" /> Other Skills</h3>
      ...
      <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
        <Link href="/intern/skills" className="text-blue-600 dark:text-blue-400 font-black text-xs uppercase tracking-widest inline-flex items-center gap-2 hover:gap-3 transition-all group">
          Go to Skills Page
