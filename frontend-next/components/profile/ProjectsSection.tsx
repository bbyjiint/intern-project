  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-3xl shadow-2xl max-w-sm w-full p-6 sm:p-8 text-center border border-gray-100 dark:border-gray-800 animate-in fade-in zoom-in duration-200">
        ...
        <h3 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white mb-3">Delete Project?</h3>
        <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-8 leading-relaxed">
          Are you sure you want to delete <span className="font-bold text-gray-900 dark:text-gray-100">"{projectName}"</span>?
        </p>
    ...
    <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-sm p-5 sm:p-8 mb-6 border border-gray-100 dark:border-gray-800 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 mb-8">
        <div className="flex items-center gap-3 text-blue-600 dark:text-blue-500">
          ...
          <h2 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white">Projects</h2>
        </div>
        <button
          onClick={() => { setEditingProject(null); setIsModalOpen(true); }}
          className="w-full sm:w-auto px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-extrabold text-sm transition-all shadow-lg shadow-blue-500/25 active:scale-95"
        >
          + Add Project
        </button>
      </div>
      ...
      {projects.length === 0 ? (
        ...
        <div className="space-y-6 sm:y-8">
          {displayedProjects.map((project: any) => {
            ...
            return (
              <div key={project.id} className="group relative border border-gray-100 dark:border-gray-800 rounded-3xl p-5 sm:p-8 bg-white dark:bg-gray-900 hover:border-blue-200 dark:hover:border-blue-900/50 transition-all">
                
                {/* Status Badge - Responsive Position */}
                <div className="mb-4 sm:absolute sm:top-8 sm:right-8">
                  <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black tracking-wider uppercase border ${hasAnyFile ? "bg-green-50 dark:bg-green-900/10 text-green-600 dark:text-green-400 border-green-100 dark:border-green-900/30" : "bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-900/30"}`}>
                ...
                <div className="mb-6 sm:pr-32">
                  ...
                  <p className="text-xs sm:text-sm font-bold text-gray-400 dark:text-gray-500 mt-2 flex flex-wrap items-center gap-x-2 gap-y-1">
                    ...
                    <span className="w-full sm:w-auto">{formatDisplayDate(project.startDate) || "N/A"} - {formatDisplayDate(project.endDate) || "Present"}</span>
                  </p>
                ...
                <div className="mb-8">
                  <h4 className="text-[10px] sm:text-[11px] font-black text-gray-400 dark:text-gray-600 uppercase tracking-widest mb-4">Project Credibility</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
                    <CredibilityItem icon="github" label="Github" active={!!project.githubUrl} url={project.githubUrl} />
                    <CredibilityItem icon="link" label="Live Demo" active={!!project.projectUrl} url={project.projectUrl} />
                    <CredibilityItem icon="file" label="Docs" active={!!project.fileUrl} url={project.fileUrl} />
                  </div>
                </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span className={`text-[11px] font-black ${active ? "text-gray-800 dark:text-gray-200" : "text-gray-400"}`}>{label}</span>
      </div>
      <div className={`w-5 h-5 rounded-full flex items-center justify-center border-2 transition-all ${active ? "border-green-500 bg-green-50 dark:bg-green-900/30" : "border-gray-200 dark:border-gray-800"}`}>
        {active && <svg className="w-3 h-3 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={4}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
      </div>
    </a>
  );
}