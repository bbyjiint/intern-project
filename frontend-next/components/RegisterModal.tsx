...
export default function RegisterModal({ isOpen, onClose, onLoginClick }: RegisterModalProps) {
  ...
  // --- ส่วนที่แก้ไข: ป้องกันการ Scroll ฉากหลัง ---
  useEffect(() => {
    if (isOpen) {
      // เมื่อ Modal เปิด: ปิดการ scroll ที่ body
      document.body.style.overflow = 'hidden'
    } else {
      // เมื่อ Modal ปิด: คืนค่าเดิม
      document.body.style.overflow = 'unset'
    }

    // Cleanup function: กันเหนียวถ้า Component ถูกถอดออกกระทันหัน
    return () => {
      document.body.style.overflow = 'unset'
    }
  }, [isOpen])
  ...
  return (
    <div
      className="fixed inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 transition-all"
      onClick={handleBackdropClick}
    >
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto relative border border-transparent dark:border-slate-800 transition-colors duration-300 animate-in zoom-in-95 duration-200">
        <div className="p-8">
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">Join us to start your journey today.</p>
          </div>

          {/* Register Form */}
          <form className="space-y-5" onSubmit={handleSubmit}>
            {error && (
              <div className="rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-sm text-red-700 dark:text-red-400 font-semibold animate-pulse">
                {error}
              </div>
            )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-12 pr-4 py-3.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-12 pr-12 py-3.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                  required
                />
                <button
                  type="button"
            ...
            <div className="relative my-8">
              ...
              <div className="relative flex justify-center text-xs uppercase tracking-widest">
                <span className="px-3 bg-white dark:bg-slate-900 text-slate-400 dark:text-slate-500 font-bold">Or register with</span>
              </div>
            </div>
