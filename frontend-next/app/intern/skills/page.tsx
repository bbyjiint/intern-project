    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
      <div className="relative bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl max-w-sm w-full p-6 sm:p-8 text-center border border-slate-100 dark:border-slate-800 animate-in fade-in zoom-in duration-200">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 text-sm sm:text-base">
          ...
          <b className="text-slate-900 dark:text-slate-200">"{skillName}"</b>?
        </p>
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            disabled={isDeleting}
            onClick={onClose}
            className="order-2 sm:order-1 flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-2xl font-black text-xs uppercase tracking-widest"
          >
            Cancel
          </button>
          <button
            disabled={isDeleting}
            onClick={onConfirm}
            className="order-1 sm:order-2 flex-1 py-4 bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-rose-600/20 active:scale-95 transition-all flex items-center justify-center"
          >
            {isDeleting ? (
...
function EvidenceBadge({ skill }: { skill: Skill }) {
  ...
  return (
    <span
      className={`flex items-center gap-2 px-3 py-1 sm:px-4 sm:py-1.5 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-full border shadow-sm ${colorClass}`}
    >
      <div
        className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${dotClass} ${label === "Not Verified" ? "" : label === "Verified" ? "" : "animate-pulse"}`}
      />
      {label}
    </span>
  );
...
export default function SkillsPage() {
  ...
  const [bulkSkillLevels, setBulkSkillLevels] = useState<
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        setIgnoredSkills(JSON.parse(stored));
      } catch (e) {}
    }
  }, []);
  ...
  const loadSkills = useCallback(async () => {
    try {
      ...
      const certSkillNames = new Set<string>(
        (profile?.certificates || []).flatMap(
          (c: any) => c.relatedSkills || c.tags || [],
        ),
      );
      const projectSkillNames = new Set<string>(
        (profile?.projects || []).flatMap(
          (p: any) => p.relatedSkills || p.skills || [],
        ),
      );
  ...
  const missingSkills = useMemo(() => {
    const userSkillNames = new Set(
      skills.map((s) => s.name.toLowerCase().trim()),
    );
    const evidenceSkills = new Map<string, string>();
    (profileData?.certificates || []).forEach((c: any) =>
      (c.relatedSkills || []).forEach((s: string) =>
        evidenceSkills.set(s.toLowerCase().trim(), s),
      ),
    );
    (profileData?.projects || []).forEach((p: any) =>
      (p.relatedSkills || []).forEach((s: string) =>
        evidenceSkills.set(s.toLowerCase().trim(), s),
      ),
    );
  ...
  const executeBulkSave = async () => {
    ...
    try {
      await Promise.all(
        bulkSelectedSkills.map((name) =>
          apiFetch("/api/candidates/skills", {
            method: "POST",
            body: JSON.stringify({
              name,
              category: "TECHNICAL",
              level: bulkSkillLevels[name] || "Beginner",
            }),
          }),
        ),
      );
  ...
  const getLevelStyles = (level: ProficiencyLevel) => {
    switch (level) {
      case "Beginner":
        return { color: "#10B981", width: "33%", bg: "bg-emerald-500" };
      case "Intermediate":
        return { color: "#3B82F6", width: "66%", bg: "bg-blue-500" };
      case "Advanced":
              ...
              {missingSkills.length > 0 && (
                <div className="mb-8 p-4 sm:p-5 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-in slide-in-from-top-4 duration-500">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-rose-800 dark:text-rose-300 font-black text-sm sm:text-[15px]">
                        You have {missingSkills.length} unlisted skills!
                      </h4>
                      <p className="text-rose-600/80 dark:text-rose-400/80 text-xs sm:text-sm font-medium">
                        Found in your projects/certificates.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => setIsBulkModalOpen(true)}
                    className="w-full md:w-auto px-6 py-2.5 bg-rose-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg shadow-rose-600/20"
                  >
                    Review & Add
                  </button>
                </div>
              ...
              <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 mb-8">
                  ...
                  <div className="flex p-1.5 bg-slate-200/50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 w-max sm:w-fit">
                    {[
                      ...
                      <button
                        ...
                        className={`px-4 sm:px-6 py-2 text-[10px] sm:text-xs font-black uppercase tracking-wider rounded-xl transition-all whitespace-nowrap ${filterTab === tab ? "bg-white dark:bg-slate-800 text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-900"}`}
                      >
                        {tab}
                      </button>
                    ))}
                ...
                <div className="flex items-center w-full xl:w-auto">
                  <select
                    ...
                    className="flex-1 sm:flex-none pl-4 pr-10 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-xs font-bold text-slate-600 cursor-pointer outline-none"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                {filteredSkills.map((skill) => {
                  ...
                  return (
                    <div
                      key={skill.id}
                      className="bg-white dark:bg-slate-900 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-200 dark:border-slate-800 p-5 sm:p-8 flex flex-col transition-all hover:shadow-xl hover:shadow-blue-500/5"
                    >
                      ...
                      <div className="mt-auto">
                        ...
                        <div className="relative w-full h-2.5 bg-slate-100 dark:bg-slate-800 rounded-full mb-6 overflow-hidden">
                          <div
                            className={`absolute h-full transition-all duration-700 ${style.bg}`}
                        ...
                        <div className="flex items-center justify-between pt-5 border-t border-slate-50 dark:border-slate-800/50">
                          ...
                          <div className="flex flex-col items-end gap-1.5">
                            ...
                            >
                              Skill Test {Math.min(skill.attemptsUsed, 3)}/3
                            </button>
                            {/* Text show when quota is maxed */}
                            {skill.attemptsUsed >= 3 &&
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-t-[2rem] sm:rounded-3xl shadow-2xl max-w-md w-full p-6 sm:p-8 border border-slate-100 dark:border-slate-800 animate-in slide-in-from-bottom sm:zoom-in duration-300 flex flex-col max-h-[90vh]">
            ...
            <div className="shrink-0 mb-4">
              ...
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                We found these skills in your certificates and projects.
              </p>
            </div>
            ...
            <div className="shrink-0 flex items-center justify-between mb-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={
                    bulkSelectedSkills.length === missingSkills.length &&
                    missingSkills.length > 0
                  }
            ...
            <div className="flex-1 overflow-y-auto space-y-2 mb-6 pr-1 min-h-[200px]">
              {missingSkills.map((skill) => {
                ...
                return (
                  <div
                    key={skill}
                    className={`flex items-center justify-between p-3 rounded-xl border transition-all ${isSelected ? "border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-900/20" : "border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50"}`}
                  >
                    <label className="flex items-center gap-3 cursor-pointer flex-1 min-w-0">
                      ...
                      />
                      <span
                        className={`text-sm font-bold truncate ${isSelected ? "text-blue-900 dark:text-blue-100" : "text-slate-700 dark:text-slate-300"}`}
                      >
                        {skill}
                      </span>
                    </label>
      ...
      {confirmBulkAction && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl max-w-sm w-full p-6 sm:p-8 text-center border border-slate-100 dark:border-slate-800 animate-in zoom-in duration-200">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 ${confirmBulkAction === "discard" ? "bg-amber-50 dark:bg-amber-500/10 text-amber-500" : "bg-blue-50 dark:bg-blue-500/10 text-blue-500"}`}
            >
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-8">
              {confirmBulkAction === "discard"
                ? "You won't see these suggestions again."
                : `Add ${bulkSelectedSkills.length} skills to your profile?`}
            </p>
