  ...
  return (
    <div className="min-h-screen bg-[#E6EBF4] dark:bg-slate-950 flex flex-col transition-colors duration-300">
      ...
      <div className="flex flex-1 relative overflow-hidden">
        {/* Sidebar พร้อมควบคุมการเปิดปิด */}
        <Sidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />

        {/* Floating Action Button (FAB) - ปรากฏเฉพาะบน Mobile/Tablet (lg:hidden) */}
        <button 
          onClick={() => setIsSidebarOpen(true)}
          className="lg:hidden fixed bottom-6 right-6 z-50 w-14 h-14 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-200 dark:border-slate-700 flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-200"
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-10">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-8 gap-6">
              <div className="flex items-center justify-between lg:block">
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm font-medium hidden sm:block">
                    View and track your recent job applications and status.
                  </p>
                </div>
                {/* ปุ่ม Mobile Menu เดิมถูกเอาออกแล้วย้ายไปที่ FAB ด้านบน */}
              </div>
            ...
            {/* Status Filter Tabs */}
            <div className="overflow-x-auto no-scrollbar mb-8 -mx-4 px-4 lg:mx-0 lg:px-0">
              <div className="flex gap-2 bg-slate-200/50 dark:bg-slate-900/50 p-1.5 rounded-2xl w-fit whitespace-nowrap">
                {["All", "Lastest", "Applied", "Accept", "Decline"].map(
                  (status) => (
                    <button
                      key={status}
                      onClick={() => setStatusFilter(status)}
                      className={`px-5 md:px-6 py-2 md:py-2.5 text-xs md:text-sm font-bold rounded-xl transition-all duration-200 ${
                        statusFilter === status
            ...
            <div className="grid grid-cols-1 gap-4 md:gap-6 sm:grid-cols-2 xl:grid-cols-3">
              {isLoading ? (
                ...
                <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 col-span-full">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 font-medium px-4">No applications found matching your criteria.</p>
                </div>
              ) : (
